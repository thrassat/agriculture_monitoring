...
* @todo
*/
// tested via postman & with real Arduino/ ardSetup.txt 
    ...
    try {  
        if (!req.params.sensorId || !req.params.groupId) {
           ...
           sendJsonResponse(res,404, {
               "message": "Parameter sensorId or groupId missing in post request"
           });
           return;
        }
        else {
                ...
                if (!sensor.confirmed) {
                    ...
                    if (dataType === "number") {
                        if (sensor.data.min && value<sensor.data.min) {
                            ...
                            // store data as int
                            // certaines valeurs peuvent être envoyées sans avoir été correctements mesurées
                            // on peut ajouter un mécanisme pour le scd30 qui en ce cas envoi 0.00 comme valeur :
                            // if (value === "0.00") alors ne pas store la valeur else la stocker comme fait ci-après
