...
// Visualisation des données disponibles pour un group de sensor donné
...
module.exports.getSensors = async function(req,res) {
    try {
        ...
        sendJsonResponse(res,200,sensors);
    }
    catch (err) {
        //console.log(err); 
        sendJsonResponse(res,404, {
            "message" : " get sensors by group id request error"
        });
        //throw error todo ? 
    }   
}
...
/****************************************************/
module.exports.getLastData = async function (req,res) {
    try {
       // console.log(req.params.sensorid); 
        let dataPacket = await storedDatas.getLastDataBySensorId(req.params.sensorid); 
        sendJsonResponse(res,200,dataPacket)  
    }
    catch (err) {
        sendJsonResponse(res,404, {
            "message" : "get last data by sensor id request error"
        });
    }
}
/********************************************************* BEGIN  OLD & TESTING PART ******************************************************************* */
//todo other page!
